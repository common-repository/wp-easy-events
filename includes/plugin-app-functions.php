	...
	if ($new_version >= '3.5.0' && $curr_version <= '3.4.0') {
		...
		if (current_user_can('manage_options')) {
			if (class_exists('Wp_Easy_Events_EDD') && class_exists('Easy_Digital_Downloads')) {
				...
				$has_edd_pr = $wpdb->get_col($wpdb->prepare($sql));
				if (!empty($has_edd_pr)) {
				...
				$has_edd_or = $wpdb->get_col($wpdb->prepare($sql));
				if (!empty($has_edd_or)) {
			...
			if (class_exists('Wp_Easy_Events_WooCommerce') && class_exists('WooCommerce')) {
				...
				$has_woo_pr = $wpdb->get_col($wpdb->prepare($sql));
				if (!empty($has_woo_pr)) {
				...
				$has_woo_or = $wpdb->get_col($wpdb->prepare($sql));
				if (!empty($has_woo_or)) {
		...
		if (sizeof($payment->downloads) > 0) {
			...
			if (!empty($myorder)) {
				foreach ($myorder as $pr_id => $qty) {
					...
					$connected = emd_p2p_type('edd_event_tickets_edd_product')->get_connected($pr_id);
					if (!empty($connected->posts)) {
						$event_id = $connected->posts[0]->ID;
						//3- create relationship with event and order
						$p2p_id = emd_p2p_type('edd_event_tickets_edd_order')->get_p2p_id($payment_id, $event_id);
						if (empty($p2p_id)) {
							emd_p2p_type('edd_event_tickets_edd_order')->connect($payment_id, $event_id, array(
	...
	if (class_exists('Wp_Easy_Events_WooCommerce') && class_exists('WooCommerce')) {
		...
		if (sizeof($order->get_items()) > 0) {
			...
			if (!empty($myorder)) {
				foreach ($myorder as $pr_id => $qty) {
					//2- see if product is related to an event
					$connected = emd_p2p_type('woo_event_tickets_woo_product')->get_connected($pr_id);
					if (!empty($connected->posts)) {
						$event_id = $connected->posts[0]->ID;
						//3- create relationship with event and order
						$p2p_id = emd_p2p_type('woo_event_tickets_woo_order')->get_p2p_id($order_id, $event_id);
						if (empty($p2p_id)) {
							emd_p2p_type('woo_event_tickets_woo_order')->connect($order_id, $event_id, array(
								'date' => current_time('mysql')
							));
...
function wp_easy_events_insert_event_attendee($cust, $qty, $event_id) {
	...
	if ($id = wp_insert_post($mypost)) {
		...
		emd_p2p_type('event_attendee')->connect($id, $event_id, array(
			'date' => current_time('mysql')
		));
