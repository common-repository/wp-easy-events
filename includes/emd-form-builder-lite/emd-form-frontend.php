		...
		if(empty($shc_list['has_form_lite']) && function_exists('emd_form_builder_process')){
			...
			if (!empty($myform) && $_POST['form_name'] == $fcontent['name']) {
				if(!empty($fcontent['settings']['enable_ajax']) || !empty($_POST['save_step']) || !empty($_POST['save_end'])){
					...
					if ($ret === false) {
						$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please refresh the page and try again.', 'wp-easy-events') . '</a></div>';
					...
					if(!empty($_POST['login_box_reg_username']) && !empty(trim($_POST['login_box_reg_password'])) && !empty(trim($_POST['login_box_reg_confirm_password']))){
						if(trim($_POST['login_box_reg_password']) == trim($_POST['login_box_reg_confirm_password'])){
							if(!validate_username(sanitize_text_field($_POST['login_box_reg_username']))){	
								wp_send_json_error(array('msg' => __('Invalid username','wp-easy-events')));
								die();
							}
							elseif(username_exists(sanitize_text_field($_POST['login_box_reg_username']))){
								wp_send_json_error(array('msg' => __('Username already taken','wp-easy-events')));
								die();
							}
							else {
								...
								if(is_wp_error($new_user_id)){
									wp_send_json_error(array('msg' => __('Please try again','wp-easy-events')));
									die();
								}
							...
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter same password.', 'wp-easy-events') . '</a></div>';
							wp_send_json_error(array('status' => 'error', 'msg' => $ret));
							die();
						}
					}
						...
						if(empty($_POST['login_box_reg_username'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter username.', 'wp-easy-events') . '</a></div>';
						}
						elseif(empty($_POST['login_box_reg_password'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter password.', 'wp-easy-events') . '</a></div>';
						}
						elseif(empty($_POST['login_box_reg_confirm_password'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter confirm password.', 'wp-easy-events') . '</a></div>';
						}
						wp_send_json_error(array('status' => 'error', 'msg' => $ret));
						die();
					}
	...
	switch($cfield['display_type']){
		...
		case 'select_advanced':
			...
			if(!empty($options)){
				foreach($options as $kopt => $vopt){
					...
					$attr_lay .= '>' . __($vopt,'wp-easy-events') . '</option>';
				}
			}
			$attr_lay .= '</select>';
			break;
		...
		case 'thickbox_image':
			...
			if (!empty($cfield['max_files'])) {
				$attr_lay .= '<div class="emd-max-file-num">' . sprintf(__('Max number of files: %s', 'wp-easy-events') , $cfield['max_files']) . '</div>';
			}
			if (!empty($cfield['max_file_size'])) {
				$attr_lay .= '<div class="emd-max-file-size">' . sprintf(__('Max file size: %s', 'wp-easy-events') , $cfield['max_file_size']) . ' KB</div>';
			}
			if (!empty($cfield['file_exts'])) {
				$attr_lay .= '<div class="emd-max-file-exts">' . sprintf(__('File extensions allowed: %s', 'wp-easy-events') , $cfield['file_exts']) . '</div>';
			}
			$attr_lay .= '</div>';
...
function emd_form_builder_lite_render_form($form_id,$app,$fcontent,$error='',$submit_result,$atts_set=''){
	...
	$local_vars['validate_msg']['required'] = __('This field is required.', 'wp-easy-events');
	$local_vars['validate_msg']['passw'] = __('Please enter same password.', 'wp-easy-events');
	$local_vars['validate_msg']['remote'] = __('Please fix this field.', 'wp-easy-events');
	$local_vars['validate_msg']['email'] = __('Please enter a valid email address.', 'wp-easy-events');
	$local_vars['validate_msg']['url'] = __('Please enter a valid URL.', 'wp-easy-events');
	$local_vars['validate_msg']['date'] = __('Please enter a valid date.', 'wp-easy-events');
	$local_vars['validate_msg']['dateISO'] = __('Please enter a valid date ( ISO )', 'wp-easy-events');
	$local_vars['validate_msg']['number'] = __('Please enter a valid number.', 'wp-easy-events');
	$local_vars['validate_msg']['digits'] = __('Please enter only digits.', 'wp-easy-events');
	$local_vars['validate_msg']['creditcard'] = __('Please enter a valid credit card number.', 'wp-easy-events');
	$local_vars['validate_msg']['equalTo'] = __('Please enter the same value again.', 'wp-easy-events');
	$local_vars['validate_msg']['maxlength'] = __('Please enter no more than {0} characters.', 'wp-easy-events');
	$local_vars['validate_msg']['minlength'] = __('Please enter at least {0} characters.', 'wp-easy-events');
	$local_vars['validate_msg']['rangelength'] = __('Please enter a value between {0} and {1} characters long.', 'wp-easy-events');
	$local_vars['validate_msg']['range'] = __('Please enter a value between {0} and {1}.', 'wp-easy-events');
	$local_vars['validate_msg']['max'] = __('Please enter a value less than or equal to {0}.', 'wp-easy-events');
	$local_vars['validate_msg']['min'] = __('Please enter a value greater than or equal to {0}.', 'wp-easy-events');
	$local_vars['unique_msg'] = __('Please enter a unique value.', 'wp-easy-events');
	$local_vars['user_email_msg'] = __('This email has been already registered.', 'wp-easy-events');
	if(!empty($fcontent['settings']['enable_ajax'])){
	...
	if($fcontent['type'] == 'submit' && !empty($ret_attrs['file_attrs'])){
		...
		foreach($ret_attrs['file_attrs'] as $myfattr){
			...
			$local_vars[$myfattr]['btnText'] = __('File Upload', 'wp-easy-events');
			...
			$local_vars[$myfattr]['errorMsg'] = __('ERROR:', 'wp-easy-events');
			$local_vars[$myfattr]['invalidExtError'] = __('Invalid file type.', 'wp-easy-events');
			$local_vars[$myfattr]['sizeError'] = __('File size is greater than allowed limit.', 'wp-easy-events');
			$local_vars[$myfattr]['maxUploadError'] = __('Maximum number of allowable file uploads has been exceeded.', 'wp-easy-events');
	...
	if($page_count == 1){
		...
		if(!empty($fcontent['settings']['submit_button_label'])){
			...
			$layout .= __('Submit','wp-easy-events');
		}
...
function emd_form_builder_lite_display_login_button($fcontent,$ent_list,$cfield,$hidden_rel=''){
	...
	$layout .= "' type='submit' value='submit' name='submit_" . $fcontent['name'] . "'>";
	$layout .= __('Login','wp-easy-events');
	$layout .= '</button>';
...
function emd_form_builder_lite_process_login(){
	$nonce_verified = wp_verify_nonce(sanitize_text_field($_POST['nonce']), 'emd_login_form');
		...
		//error
		$error = __('Please refresh the page and try again.','wp-easy-events');
	}
	else {
		$error = "";
		...
		if($user_data) {
			...
			if(wp_check_password(sanitize_text_field($_POST['emd_user_pass']), $user_data->user_pass, $user_data->ID)) {
				...
				$error = __( 'The password or username you entered is incorrect.', 'wp-easy-events');
			}
		} else {
			$error = __('The password or username you entered is incorrect.', 'wp-easy-events');
		}
