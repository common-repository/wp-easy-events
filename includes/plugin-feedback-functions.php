...
function wp_easy_events_show_optin() {
	...
	if (!get_option('wp_easy_events_tracking_optin')) {
		...
		echo '<p>' . esc_html__('Please confirm your email address below to start receiving emails from us.', 'wp-easy-events') . '</p>';
		$current_user = wp_get_current_user();
		...
		echo wp_kses($tr_link, $allowed) . '</ul><div class="emd-permissions"><a href="#" class="emd-perm-trigger"><span class="dashicons dashicons-info" style="text-decoration:none;"></span>' . esc_html__('What permissions are being granted?', 'wp-easy-events') . '</a><ul class="emd-permissions-list" style="display:none;">';
		echo '<li class="emd-permission"><i class="dashicons dashicons-nametag"></i><div><span>' . esc_html__('Your Profile Overview', 'wp-easy-events') . '</span><p>' . esc_html__('Name and email address', 'wp-easy-events') . '</p></div></li>';
		echo '<li class="emd-permission"><i class="dashicons dashicons-admin-settings"></i><div><span>' . esc_html__('Your Site Overview', 'wp-easy-events') . '</span><p>' . esc_html__('Site URL, WP version and PHP info', 'wp-easy-events') . '</p></div></li>';
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . esc_html__('Newsletter', 'wp-easy-events') . '</span><p>' . esc_html__('Updates, announcements, marketing, no spam', 'wp-easy-events') . ', <a href="https://emdplugins.smartlamb.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
...
function wp_easy_events_show_rateme_action() {
	if (!wp_verify_nonce(sanitize_text_field($_POST['rateme_nonce']) , 'wp_easy_events_rateme_nonce')) {
		exit;
	}
	$min_trigger = get_option('wp_easy_events_show_rateme_plugin_min', 10);
	if ($min_trigger == - 1) {
...
function wp_easy_events_deactivation_feedback_box() {
	...
	$reasons[7] = __('Other', 'wp-easy-events');
...
function wp_easy_events_send_deactivate_reason() {
	...
	if (!wp_verify_nonce(sanitize_text_field($_POST['deactivate_nonce']) , 'wp_easy_events_deactivate_nonce')) {
		exit;
	}
	$uemail = '';
