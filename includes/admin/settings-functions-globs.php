...
function emd_glob_settings_tab($tabs,$app){
	...
	if(!empty($init_variables)){
		$tabs['global'] = __('Global', 'wp-easy-events');
	...
	if(!empty($variables)){
		foreach($variables as $kv => $val){
			...
			if($val['required'] == 1 && empty($input[$kv])){
				$error_message = sprintf(__( "%s is required.", 'wp-easy-events'),$val['label']);
	...
	foreach($init_variables as $id => $myvar){
		...
		switch($myvar['type']){
			...
			case 'map':
				...
				echo "</select></td></tr>";
				echo "<tr><th scope='row'><label for='" . esc_attr($id) . "_map_type'>" . esc_html__('Type','wp-easy-events') . "</th><td><select id='" . esc_attr($app) . "_glob_list_" . esc_attr($id) . "_map_type' name='" . esc_attr($app) . "_glob_list[" . esc_attr($id) . "_map_type]'>";
				...
				echo "</select></td></tr>";
				echo "<tr><th scope='row'><label for='" . esc_attr($id) . "_marker'>" . esc_html__('Marker','wp-easy-events') . "</th><td><input id='" . esc_attr($app) . "_glob_list_" . esc_attr($id) . "_marker' name='" . esc_attr($app) . "_glob_list[" . esc_attr($id) . "_marker]' type='checkbox' value=1 " . esc_attr($marker) . "></input></td></tr>";
				echo "<tr><th scope='row'><label for='" . esc_attr($id) . "_marker_title'>" . esc_html__('Marker Title','wp-easy-events') . "</th><td><input id='" . esc_attr($app) . "_glob_list_" . esc_attr($id) . "_marker_title' name='" . esc_attr($app) . "_glob_list[" . esc_attr($id) . "_marker_title]' type='text' value='" . esc_attr($marker_title) ."'></input><p class='description'>" . esc_html__('Sets the marker title when hover.','wp-easy-events') . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='" . esc_attr($id) . "_info_window'>" . esc_html__('Info Window','wp-easy-events') . "</th><td><textarea id='" . esc_attr($app) . "_glob_list_" . esc_attr($id) . "_info_window' name='" . esc_attr($app) . "_glob_list[" . esc_attr($id) . "_info_window]'>" . esc_attr($info_window) . "</textarea><p class='description'>" . esc_html__('Sets the content of the info box. You can use html tags.','wp-easy-events') . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='" . esc_attr($id) . "_load_info'>" . esc_html__('Display Info Window on Page Load','wp-easy-events') . "</th><td><input id='" . esc_attr($app) . "_glob_list_" . esc_attr($id) . "_load_info' name='" . esc_attr($app) . "_glob_list[" . esc_attr($id) . "_load_info]' type='checkbox' value=1 " . esc_attr($load_info) . "></input></td></tr>";
				echo "<tr><th><p class='description'>" . esc_html__('You can drag and drop the marker to specify the exact location.','wp-easy-events') . "</th><td><div class='emd-mb-map-field'><div class='emd-mb-map-canvas' data-default-loc=''></div>
