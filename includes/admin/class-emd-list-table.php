...
class Emd_List_Table extends WP_List_Table {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$columns = array(
					...
					'name' => esc_html__( 'Name', 'wp-easy-events' ),
					'label' => esc_html__( 'Label', 'wp-easy-events' ),
					'entity' => esc_html__( 'Entity', 'wp-easy-events' ),
					'ctype' => esc_html__( 'Type', 'wp-easy-events' ),
					'dtype' => esc_html__( 'Display Type', 'wp-easy-events' ),
					'created'   => esc_html__( 'Updated', 'wp-easy-events' ),
				);
			}
			else {
				$columns = array(
					'cb'        => '<input type="checkbox" />',
					'name' => esc_html__( 'Name', 'wp-easy-events' ),
					'type' => esc_html__( 'Type', 'wp-easy-events' ),
					'shortcode' => esc_html__( 'Shortcode', 'wp-easy-events' ),
					'created'   => esc_html__( 'Created', 'wp-easy-events' ),
				);
				if($this->type == 'form'){
					$columns['modified'] = esc_html__( 'Modified', 'wp-easy-events' );
				}
			}
		}
		else {
	...
	public function column_default( $rec, $column_name ) {
		switch ( $column_name ) {
			...
			case 'ctype':
				if($rec['ctype'] == 'attr'){
					$value = __('Attribute','wp-easy-events');
				}
				elseif($rec['ctype'] == 'tax'){
					$value = __('Taxonomy','wp-easy-events');
				}
				break;
			case 'dtype':
				$value = $rec['dtype'];
			...
			case 'shortcode':
				$value = '<p id="shortcode_' . $rec['id'] . '">' . stripslashes($rec['shortcode']) . '</p>
					<a class="emd-copy-clipb button button-primary" style="padding:1px 6px;" data-clipboard-target="#shortcode_' . $rec['id'] . '">' . __('Copy','wp-easy-events') . '</a>';
				break;
			case 'created':
	...
	public function get_bulk_actions() {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$actions = array(
					'delete' => esc_html__( 'Delete Field', 'wp-easy-events' ),
					'delete_data' => esc_html__( 'Delete Field & Data', 'wp-easy-events' ),
				);
			}
			elseif($this->type == 'form'){
				$actions = array(
					'delete' => esc_html__( 'Delete', 'wp-easy-events' ),
					'export' => esc_html__( 'Export', 'wp-easy-events' ),
				);
			}
			else {
				$actions = array(
			...
			if (wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'bulk-' . $this->type . 's')){
				...
				<div class="notice updated">
					<p>
						<?php
						if ( count( $ids ) === 1 ) {
							esc_html_e( 'Record was successfully deleted.', 'wp-easy-events' );
						} else {
							esc_html_e( 'Records were successfully deleted.', 'wp-easy-events' );
						}
						?>
					</p>
				</div>
				...
				<div class="notice updated">
					<p>
						<?php esc_html_e( 'Security check failed. Please try again.', 'wp-easy-events' ); ?>
					</p>
				</div>
				<?php
			}
	...
	/**
		...
		if($this->type == 'form'){
			$name = sprintf(
				...
				esc_html__('Edit Form', 'wp-easy-events'),
				$name
			);
			...
			$row_actions['edit_settings'] = sprintf(
				...
				esc_html__('Edit Settings', 'wp-easy-events'),
				esc_html__('Edit Settings', 'wp-easy-events')
			);
			$row_actions['edit'] = sprintf(
				...
				esc_html__('Edit Layout', 'wp-easy-events'),
				esc_html__('Edit Layout', 'wp-easy-events')
			);
		...
		if($this->type == 'cfield'){
			$name = sprintf(
				...
				esc_html__('Edit', 'wp-easy-events'),
				$name
			);
		}
		return $name;
