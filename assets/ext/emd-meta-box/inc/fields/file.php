	...
	class EMD_MB_File_Field extends EMD_MB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'emd-mb-file', 'emdmbFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'wp-easy-events' ),
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'wp-easy-events' ),
				'maxFileSizeError' => __('File Size is greater than allowed limit.','wp-easy-events'),
				'FileExtError'	=> __('Invalid file type.','wp-easy-events'),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( __( 'Error: Cannot delete file', 'wp-easy-events' ) );
		}
		...
		{
			$i18n_title = apply_filters( 'emd_mb_file_upload_string', _x( 'Upload Files', 'file upload', 'wp-easy-events' ), $field );
			$i18n_more  = apply_filters( 'emd_mb_file_add_string', _x( '+ Add new file', 'file upload', 'wp-easy-events' ), $field );
			...
			if(!empty($field['max_file_uploads'])){
				$file_settings .= sprintf(__('Max number of files: %s','wp-easy-events'),$field['max_file_uploads']);
			}
			if(!empty($field['max_file_size'])){
				$file_settings .= '<br> ' . sprintf(__('Max file size: %s','wp-easy-events'),$field['max_file_size']) . ' KB';
			}
			else {
				...
				$file_settings .= '<br> ' . sprintf(__('Max file size: %s','wp-easy-events'),$server_size) . ' KB';
			}
			if(!empty($field['mime_type'])){
				$file_settings .= '<br> ' . sprintf(__('File extensions allowed: %s','wp-easy-events'),$field['mime_type']);
			}
			// Show form upload
			$html .= sprintf(
				'<div class="%s">
		...
		{
			$i18n_delete = apply_filters( 'emd_mb_file_delete_string', _x( 'Delete', 'file upload', 'wp-easy-events' ) );
			$i18n_edit   = apply_filters( 'emd_mb_file_edit_string', _x( 'Edit', 'file upload', 'wp-easy-events' ) );
			$li = '
			<li>
