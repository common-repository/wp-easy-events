...
class EmdScbUtil {
	...
	// Enable delayed activation; to be used with emd_scb_init()
	static function add_activation_hook( $plugin, $callback ) {
		if ( defined( 'SCB_LOAD_MU' ) )
			register_activation_hook( $plugin, $callback );
		else
			add_action( 'emd_scb_activation_' . plugin_basename( $plugin ), $callback );
	}

	// For debugging
	static function do_activation( $plugin ) {
		do_action( 'emd_scb_activation_' . plugin_basename( $plugin ) );
	}
	...
	static function objects_to_assoc( $objects, $key, $value ) {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'r41', 'emd_p2p_scb_list_fold()' );
		return emd_p2p_scb_list_fold( $objects, $key, $value );
	}
...
function emd_p2p_scb_admin_notice( $msg, $class = 'updated' ) {
	return emd_p2p_html( "div class='$class fade'", emd_p2p_html( "p", $msg ) );
}

// Transform a list of objects into an associative array
function emd_p2p_scb_list_fold( $list, $key, $value ) {
	$r = array();
 ...
 */
if ( ! function_exists( 'emd_p2p_html' ) ):
function emd_p2p_html( $tag ) {
...
// Generate an <a> tag
if ( ! function_exists( 'emd_p2p_html_link' ) ):
function emd_p2p_html_link( $url, $title = '' ) {
	if ( empty( $title ) )
		$title = $url;

	return emd_p2p_html( 'a', array( 'href' => $url ), $title );
}
endif;

function emd_p2p_scb_get_query_flags( $wp_query = null ) {
	if ( !$wp_query )
...
// WP < ?
if ( ! function_exists( 'emd_p2p_set_post_field' ) ) :
function emd_p2p_set_post_field( $field, $value, $post_id ) {
	global $wpdb;
