 ...
 *
 * Emd_Mustache is a framework-agnostic logic-less templating language. It enforces separation of view
...
class Emd_Mustache {
	...
	/**
	 * Should this Emd_Mustache throw exceptions when it finds unexpected tags?
	 *
	 * @see self::_throwsException()
	 */
	protected $_throwsExceptions = array(
		Emd_MustacheException::UNKNOWN_VARIABLE         => false,
		Emd_MustacheException::UNCLOSED_SECTION         => true,
		Emd_MustacheException::UNEXPECTED_CLOSE_SECTION => true,
		Emd_MustacheException::UNKNOWN_PARTIAL          => false,
		Emd_MustacheException::UNKNOWN_PRAGMA           => true,
	);
	 ...
	 *         'pragmas' => array(
	 *             Emd_Mustache::PRAGMA_UNESCAPED => true
	 *         ),
	 *
		...
		if (isset($options['pragmas'])) {
			foreach ($options['pragmas'] as $pragma_name => $pragma_value) {
				if (!in_array($pragma_name, $this->_pragmasImplemented, true)) {
					throw new Emd_MustacheException('Unknown pragma: ' . $pragma_name, Emd_MustacheException::UNKNOWN_PRAGMA);
				}
			}
	...
	public function __clone() {
		...
		if ($keys = array_keys($this->_context)) {
			$last = array_pop($keys);
			if ($this->_context[$last] instanceof Emd_Mustache) {
	 ...
	 * @return string Rendered Emd_Mustache template.
	 */
	...
	protected function _findSection($template) {
			...
			switch ($type) {
				...
				case '/':
					if (empty($section_stack) || ($tag_name !== array_pop($section_stack))) {
						if ($this->_throwsException(Emd_MustacheException::UNEXPECTED_CLOSE_SECTION)) {
							throw new Emd_MustacheException('Unexpected close section: ' . $tag_name, Emd_MustacheException::UNEXPECTED_CLOSE_SECTION);
						}
					}
		...
		if (!empty($section_stack)) {
			if ($this->_throwsException(Emd_MustacheException::UNCLOSED_SECTION)) {
				throw new Emd_MustacheException('Unclosed section: ' . $section_stack[0], Emd_MustacheException::UNCLOSED_SECTION);
			}
		}
	}
	 ...
	 * @param mixed $matches
	 * @return void
	 * @throws Emd_MustacheException unknown pragma
	 */
	protected function _renderPragma($matches) {
		...
		if (!in_array($pragma_name, $this->_pragmasImplemented)) {
			if ($this->_throwsException(Emd_MustacheException::UNKNOWN_PRAGMA)) {
				throw new Emd_MustacheException('Unknown pragma: ' . $pragma_name, Emd_MustacheException::UNKNOWN_PRAGMA);
			} else {
				return '';
			}
	 ...
	 *
	 * Expects exceptions to be Emd_MustacheException error codes (i.e. class constants).
	 *
	 * @access protected
	 ...
	 * @throws Emd_MustacheException Unmatched section tag encountered.
	 * @return string
	 */
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws Emd_MustacheException Unknown variable name.
	 * @return string
	 */
	...
	protected function _findVariableInContext($tag_name, $context) {
		...
		if ($this->_throwsException(Emd_MustacheException::UNKNOWN_VARIABLE)) {
			throw new Emd_MustacheException("Unknown variable: " . $tag_name, Emd_MustacheException::UNKNOWN_VARIABLE);
		} else {
			return '';
		}
	}
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws Emd_MustacheException Unknown partial name.
	 * @return string
	 */
	protected function _getPartial($tag_name) {
		...
		if ($this->_throwsException(Emd_MustacheException::UNKNOWN_PARTIAL)) {
			throw new Emd_MustacheException('Unknown partial: ' . $tag_name, Emd_MustacheException::UNKNOWN_PARTIAL);
		} else {
			return '';
		}
	}
...
/**
 * Emd_MustacheException class.
 *
 * @extends Exception
 */
class Emd_MustacheException extends Exception {
