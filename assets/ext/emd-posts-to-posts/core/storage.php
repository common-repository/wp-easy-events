...
class P2P_Storage {
	...
	static function init() {
		emd_scb_register_table( 'p2p' );
		emd_scb_register_table( 'p2pmeta' );
	...
	static function uninstall() {
		emd_scb_uninstall_table( 'p2p' );
		emd_scb_uninstall_table( 'p2pmeta' );
		...
		foreach ( P2P_Connection_Type_Factory::get_all_instances() as $p2p_type => $ctype ) {
			foreach ( array( 'from', 'to' ) as $direction ) {
				if ( $object_type == $ctype->side[ $direction ]->get_object_type() ) {
					emd_p2p_delete_connections( $p2p_type, array(
						$direction => $object_id,
					) );
				}
			}
