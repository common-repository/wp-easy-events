...
class P2P_Directed_Connection_Type {
	...
	public function flip_direction() {
		return $this->set_direction( emd_p2p_flip_direction( $this->direction ) );
	}
	...
	public function get_related( $item, $extra_qv = array(), $output = 'raw' ) {
		...
		$additional_qv = array( 'p2p:exclude' => emd_p2p_normalize( $item ) );
	...
	public function get_connectable( $arg, $extra_qv = array(), $output = 'raw' ) {
		...
		if(!empty($item)){
			...
			$qv = apply_filters( 'p2p_connectable_args', $extra_qv, $this,"");
		}
	...
	protected function get_non_connectable( $item, $extra_qv ) {
		...
		emd_p2p_append( $to_exclude, $already_connected );
	...
	public function connect( $from_arg, $to_arg, $meta = array() ) {
		...
		foreach ( $this->fields as $key => $args ) {
			...
			foreach ( (array) $this->get_default( $args, $p2p_id ) as $default_value ) {
				emd_p2p_add_meta( $p2p_id, $key, $default_value );
			}
		}
	...
	protected function get_default( $args, $p2p_id ) {
		if ( isset( $args['default_cb'] ) )
			return call_user_func( $args['default_cb'], emd_p2p_get_connection( $p2p_id ), $this->direction );
	...
	public function get_p2p_id( $from, $to ) {
		return emd_p2p_first( $this->get_connections( array(
			'from' => $from,
			'to' => $to,
			'fields' => 'p2p_id'
		) ) );
	...
	/**
	 * Transforms $this->get_connections( ... ) into emd_p2p_get_connections( $this->name, ... ) etc.
	 */
	public function __call( $method, $argv ) {
		...
		return call_user_func( 'emd_p2p_' . $method, $this->name, $args );
	}
}
